...
func Test(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run("testcase", func(t *testing.T) {
			...
			got := lowestCommonAncestor(bTree, &TreeNode{Val: tc.p}, &TreeNode{Val: tc.q})
			want := &TreeNode{Val: tc.want}

			if got.Val != want.Val {
				t.Fatalf("got %v want %v", got.Val, want.Val)
			}
		})
	}
}
